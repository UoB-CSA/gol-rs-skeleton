...
pub async fn remote_distributor(params: Params, channels: DistributorChannels) -> Result<()> {
...
async fn example_rpc_call(server_addr: String) -> Result<()> {
    ...
    let response = client
        .push_world(tonic::Request::new(World {
            width: 3,
            height: 3,
            cell_values: bytes,
        }))
    ...
    client
        .shutdown_broker(tonic::Request::new(Empty {}))
        .await?;
